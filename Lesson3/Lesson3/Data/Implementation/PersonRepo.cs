...
namespace Lesson3.Data.Implementation
{
    ...
    {        
        private static List<Person> data = new List<Person>()
        {
             ...
             new Person { Id = 3, FirstName = "Veda", LastName = "Holmes", Email = "neque.Sed.eget@non.co.uk", Company = "Et Associates", Age = 63 },
        ...
        {
            var result = data.FindAll(x => x.FirstName == name);
            return result;
        }

        public IEnumerable<Person> GetSkip(int skip, int take)
        {
            var result = data.FindAll(x => x.Id>=skip && x.Id <=take);
            return result;
        }

        public void Update(Person update)
        {
            var result = data.Find(x => x.Id == update.Id);
            if (result != null)
            {
        ...
        public void Delete(int id)
        {
            ...
            if (result != null)
            {
                for (var i = data.IndexOf(result); i<data.Count;i++)
                {
